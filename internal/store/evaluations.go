...
func (s *Store) LatestEvaluations(stationID string, n int) ([]model.StabilityEvaluation, error) {
	rows, err := s.db.Query(`SELECT id,station_id,score,danger_level,weak_layer_idx,weak_layer_depth_cm,wind_factor,created_at
		FROM evaluations WHERE station_id=? ORDER BY created_at DESC LIMIT ?`, stationID, n)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) EvaluationTrend(stationID string, n int) ([]float64, error) {
	rows, err := s.db.Query(`SELECT score FROM (SELECT score,created_at FROM evaluations WHERE station_id=? ORDER BY created_at DESC LIMIT ?) ORDER BY created_at`, stationID, n)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CountEvaluationsSince(since time.Time) (int, error) {
	var n int
	err := s.db.QueryRow(`SELECT COUNT(*) FROM evaluations WHERE created_at>=?`, fmtTime(since)).Scan(&n)
	return n, err
}
